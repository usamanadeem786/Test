...
from auth.repositories import UserFieldRepository
...
class TestCreateUserField:
    ...
    async def test_already_existing_slug(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "user_field_slug_already_exists"
...
class TestDeleteUserField:
    ...
    async def test_valid_get(
        ...
        submit_button = html.find(
            "button",
            attrs={
                "hx-delete": f"http://api.auth.dev/user-fields/{user_field.id}/delete"
            },
        )
