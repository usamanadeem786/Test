...
from auth import __version__, schemas
from auth.db import AsyncSession
from auth.models import WebhookLog
from auth.repositories import WebhookLogRepository
...
class TestWebhookDelivery:
    async def test_deliver_success(
        ...
        assert "X-Auth-Webhook-Signature" in request.headers
        assert "X-Auth-Webhook-Timestamp" in request.headers
        assert request.content == webhook_event.model_dump_json().encode("utf-8")

        assert request.headers["user-agent"] == f"auth-server-webhooks/{__version__}"
