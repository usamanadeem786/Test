...
from auth.models import RegistrationSessionFlow, UserField, UserFieldType
...
from auth.tasks import on_after_register
...
class TestGetRegister:
    ...
    async def test_disabled_registration_tenant(
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == "registration_disabled"
...
class TestPostRegister:
    ...
    async def test_existing_user(
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == "user_already_exists"
    ...
    async def test_new_user_oauth_override_default_email(
        ...
        response = await test_client_auth_csrf.post(
            "/register",
            data={
                "email": "louis+auth@bretagne.duchy",
                "csrf_token": csrf_token,
            },
            cookies=cookies,
        )
        ...
        user = await user_repository.get_by_id(session_token.user_id)
        assert user is not None
        assert user.email == "louis+auth@bretagne.duchy"
        assert user.email_verified is False
