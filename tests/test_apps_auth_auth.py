...
from auth.services.response_type import DEFAULT_RESPONSE_MODE, HYBRID_RESPONSE_TYPES
...
class TestAuthAuthorize:
    ...
    async def test_client_tenant_mismatch(
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == "invalid_client"
...
class TestAuthPostLogin:
    ...
    async def test_bad_credentials(
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == "bad_credentials"
...
class TestAuthPostVerifyEmail:
    ...
    async def test_invalid_code(
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == "invalid_code"
...
class TestAuthLogout:
    async def test_missing_redirect_uri(
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == "invalid_request"
