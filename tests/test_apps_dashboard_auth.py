...
from auth_client import AuthAccessTokenMissingPermission
...
class TestAuthLogin:
    async def test_success(
        ...
        auth_client_mock.auth_url.side_effect = AsyncMock(
        ...
        auth_client_mock.auth_url.assert_called_once_with(
            redirect_uri="http://api.auth.dev/auth/callback",
            scope=["openid"],
...
class TestAuthCallback:
    ...
    async def test_missing_permission(
        self, test_client_dashboard: httpx.AsyncClient, auth_client_mock: MagicMock
    ):
        auth_client_mock.auth_callback.side_effect = AsyncMock(
            return_value=(
                {"access_token": "ACCESS_TOKEN", "id_token": "ID_TOKEN"},
                {"email": "anne@bretagne.duchy"},
            )
        )
        auth_client_mock.validate_access_token.side_effect = (
        ...
        session_cookie = response.cookies.get(settings.auth_admin_session_cookie_name)
...
class TestAuthProfile:
    ...
    async def test_valid(self, test_client_dashboard: httpx.AsyncClient):
        ...
        location = response.headers["Location"]
        assert location == f"//{settings.auth_domain}"
...
class TestAuthLogout:
    ...
    async def test_valid(
        ...
        auth_client_mock.base_url = "https://bretagne.auth.dev"
        response = await test_client_dashboard.get("/auth/logout")
        ...
        assert (
            location
            == f"//{settings.auth_domain}/logout?redirect_uri=http://api.auth.dev/admin/"
        )
