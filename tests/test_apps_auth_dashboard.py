...
class TestAuthUpdateProfile:
    async def test_unauthorized(
        ...
        assert (
            ...
            == f"http://api.auth.dev{tenant_params.path_prefix}/login"
        )
    ...
    async def test_authenticated_on_another_tenant(
        ...
        assert response.status_code == status.HTTP_307_TEMPORARY_REDIRECT
        assert response.headers["Location"] == "http://api.auth.dev/login"
...
class TestAuthUpdatePassword:
    ...
    async def test_update_invalid_old_password(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "invalid_old_password"
    ...
    async def test_update_new_passwords_dont_match(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "passwords_dont_match"
...
class TestAuthEmailChange:
    ...
    async def test_post_invalid_current_password(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "invalid_current_password"
...
class TestAuthEmailVerify:
    ...
    async def test_post_invalid_code(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "invalid_code"
