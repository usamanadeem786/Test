...
class TestCreateUser:
    ...
    async def test_unknown_tenant(
        ...
        assert response.headers["X-Auth-Error"] == "unknown_tenant"
    ...
    async def test_existing_user(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "user_already_exists"
    ...
    async def test_invalid_password(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "invalid_password"
    ...
    async def test_valid(
        ...
        user = await user_repository.get_by_id(
            uuid.UUID(response.headers["X-Auth-Object-Id"])
        )
...
class TestCreateUserAccessToken:
    ...
    async def test_unknown_client(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "unknown_client"
...
class TestDeleteUser:
    ...
    async def test_valid_get(
        ...
        submit_button = html.find(
            "button",
            attrs={"hx-delete": f"http://api.auth.dev/users/{user.id}/delete"},
        )
...
class TestUserPermissions:
    ...
    async def test_create_permission_unknown(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "unknown_permission"
    ...
    async def test_create_permission_already_added(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "already_added_permission"
...
class TestUserRoles:
    ...
    async def test_create_role_unknown(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "unknown_role"
    ...
    async def test_create_role_already_added(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "already_added_role"
