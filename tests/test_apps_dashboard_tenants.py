...
class TestCreateTenant:
    ...
    async def test_unknown_theme(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "unknown_theme"
    ...
    async def test_unknown_oauth_provider(
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.headers["X-Auth-Error"] == "unknown_oauth_provider"
...
class TestDeleteTenant:
    ...
    async def test_valid_get(
        ...
        submit_button = html.find(
            "button",
            attrs={"hx-delete": f"http://api.auth.dev/tenants/{tenant.id}/delete"},
        )
