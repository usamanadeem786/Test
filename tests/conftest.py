...
from auth.db.engine import create_engine
from auth.db.migration import migrate_schema
from auth.db.types import DatabaseConnectionParameters, DatabaseType, get_driver
...
from auth.dependencies.theme import get_theme_preview
from auth.models import AdminAPIKey, AdminSessionToken, User
from auth.services.tenant_email_domain import TenantEmailDomain
from auth.services.theme_preview import ThemePreview
...
def get_test_database(worker_id: str) -> GetTestDatabase:
    ...
    async def _get_test_database(
        *, name: str = "auth-test", drop: bool = True
...
@pytest_asyncio.fixture
async def auth_client_mock() -> MagicMock:
    return MagicMock(spec=AuthAsync)
    ...
    async def _authenticated_admin(client: httpx.AsyncClient) -> httpx.AsyncClient:
        ...
        if marker:
            ...
            if mode == "session":
                ...
                client.cookies.set(settings.auth_admin_session_cookie_name, token)
            elif mode == "api_key":
...
async def test_client_generator(
    ...
    async def _test_client_generator(app: FastAPI):
        ...
        app.dependency_overrides[get_auth] = lambda: auth_client_mock
        ...
        settings.auth_admin_session_cookie_domain = ""

        async with asgi_lifespan.LifespanManager(app):
            async with httpx.AsyncClient(
                transport=httpx.ASGITransport(app=app), base_url="http://api.auth.dev"
            ) as test_client:
