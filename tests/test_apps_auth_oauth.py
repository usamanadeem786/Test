...
from auth.models import RegistrationSessionFlow
...
class TestOAuthAuthorize:
    ...
    async def test_unknown_tenant_id(
        ...
        assert headers["X-Auth-Error"] == "invalid_tenant"
    ...
    async def test_invalid_login_session(
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == "invalid_session"
    ...
    async def test_invalid_oauth_provider(
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == "invalid_provider"
    ...
    async def test_expired_login_session(
        ...
        assert location.startswith(f"http://api.auth.dev{path_prefix}/authorize")
...
class TestOAuthCallback:
    ...
    async def test_redirect_error(
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == error
    ...
    async def test_access_token_error(
        ...
        mocker.patch(
            "auth.apps.auth.routers.oauth.get_oauth_provider_service"
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == "access_token_error"
    ...
    async def test_get_id_email_error(
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == "id_email_error"
    ...
    async def test_existing_oauth_account_inactive(
        ...
        headers = response.headers
        assert headers["X-Auth-Error"] == "inactive_user"
